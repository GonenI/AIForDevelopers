...
def build_vocab(sentences, min_freq=1):
    ...
    vocab = {'<PAD>': 0, '<UNK>': 1, '<BOS>': 2, '<EOS>': 3}  # No <SEP> needed
    index = 4
    ...
    print(f"Top 10 frequent words: {sorted(word_counts.items(), key=lambda x: x[1], reverse=True)[:10]}")
    return vocab
...
class SentenceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        combined_text = f"<BOS> {sentence} <EOS>"
...
def predict(input_text, model, vocab, seq_len=20, max_words=5, temperature=0.9, top_k=15):
    reverse_vocab = {v: k for k, v in vocab.items()}
    input_tokens = ['<BOS>'] + word_tokenize(input_text)
...
sentences = load_sentences('sentences.txt')  # Updated function name
...
model = GPTModel(vocab_size=len(vocab), seq_len=seq_len).to(DEVICE)
